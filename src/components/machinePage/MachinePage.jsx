...
const MachinePage = () => {
  ...
  const machineData = Object.values(solutionsByMachineType)
    ...
    .find((type) => 
      Object.values(type).some(
        (machine) => 
          machine.title &&
          machine.title.toLowerCase() === decodeURIComponent(machineTitle).toLowerCase()
      )
    );
  ...
  const machine = machineData
    ? Object.values(machineData).find(
        (m) =>
          m.title &&
          m.title.toLowerCase() === decodeURIComponent(machineTitle).toLowerCase()
      )
    : null;
  ...
  const images = machine.image
    ? (Array.isArray(machine.image) ? machine.image : [machine.image])
    : [];
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pt-32">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          <h1 className="text-3xl font-bold text-gray-800">{machine.title}</h1>
          <p className="text-lg text-gray-600">{machine.description}</p>

          <h2 className="text-2xl font-semibold text-gray-700">Características:</h2>
